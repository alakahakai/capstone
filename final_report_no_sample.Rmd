---
title: "Exploratory Analysis of the Capstone Dataset - No Sampling"
subtitle: "Generating data for final app"
author: "Ray Qiu"
...
##### Take sample data from the dataset, 5% sampling rate
...
wordcloud(names(freq), freq, max.words=100,
          scale=c(2,.5), colors=brewer.pal(8, "Dark2"))
...
freqDF4 <- data.frame(name = names(freq4), freq = freq4, row.names = NULL, 
                      stringsAsFactors = FALSE)
freqDF4_top20 <- head(freqDF4, 20)

# Plot words in frequency order
ggplot(aes(x = reorder(name, -freq), y = freq, fill = freq), data = freqDF4_top20) + 
  geom_histogram(stat = "identity") +
  labs(y="Frequency", title="Top 20 Quadgrams in samples") +
  theme(axis.title.x = element_blank(), 
        axis.text.x = element_text(angle = 60, size = 8, hjust = 1))
# Display wordcloud
wordcloud(names(freq4), freq4, max.words=50, 
          scale=c(2,.25), colors=brewer.pal(8, "Dark2"))

saveRDS(freqDF, file = "app/data/unigram.RDS")
saveRDS(freqDF2, file = "app/data/bigram.RDS")
saveRDS(freqDF3, file = "app/data/trigram.RDS")
saveRDS(freqDF4, file = "app/data/quadgram.RDS")
```
